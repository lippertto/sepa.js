...
import {_SepaXmlNode} from './sepa.js';
...
describe('XML tests', () => {
  test('Nodes are written out as self-closing', () => {
    ...
    const sut = new _SepaXmlNode('node');
  ...
  test('Adds text', () => {
    ...
    const sut = new _SepaXmlNode('node', 'some-text');
    // WHEN we serialize the xml tree
    const xmlString = sut.toString(0, false);
    const dom = new xmldom.DOMParser().parseFromString(xmlString, 'text/xml');
  ...
  test('Adds children', () => {
    ...
    const parent = new _SepaXmlNode('parent');
    parent.appendChild(new _SepaXmlNode('child'));
    // WHEN we serialize the xml tree
  ...
  test('Does not allow cdata as text value', () => {
    ...
    const sut = new _SepaXmlNode('node', '<![CDATA[text]]>');
    // WHEN we serialize the xml tree
    const xmlString = sut.toString(0, false);
  ...
  test.each([
    ...
    expect(() => new _SepaXmlNode('node', text)).toThrow();
  });

  test('Uses attributes from constructor', () => {
    // GIVEN a node with attributes set in the constructor
    const sut = new _SepaXmlNode('node', 'text', {key1: 'value1'});
  ...
  test('Does not allow invalid node names', () => {
    ...
    expect(() => {new _SepaXmlNode(badName);}).toThrow();
  });
  ...
  test('node with both children and text is rejected', () => {
    ...
    const parent = new _SepaXmlNode('p');
    parent.appendChild(new _SepaXmlNode('c'));
  ...
  test('pretty printing includes indentation and newlines', () => {
    const parent = new _SepaXmlNode('p');
    parent.appendChild(new _SepaXmlNode('c1'));
    parent.appendChild(new _SepaXmlNode('c2'));
