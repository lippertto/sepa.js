...
describe('Dates are not converted to UTC', () => {
  ...
  test('dates stay the same during xml generation for direct debit documents', () => {
    ...
    const xml = doc.toXML();
    const generatedCreatedDate = select('/p:Document/p:CstmrDrctDbtInitn/p:GrpHdr/p:CreDtTm/text()', xml, true);
    expect(generatedCreatedDate.toString()).toBe('2023-07-29T01:02:03');

    const generatedCollectionDate = select('/p:Document/p:CstmrDrctDbtInitn/p:PmtInf/p:ReqdColltnDt/text()', xml, true);
    ...
    const generatedMandateSignatureDate = select('/p:Document/p:CstmrDrctDbtInitn/p:PmtInf/p:DrctDbtTxInf/p:DrctDbtTx/p:MndtRltdInf/p:DtOfSgntr/text()', xml, true);
  ...
  test('dates stays the same during xml generation for transfer documents', () => {
    ...
    const xml = doc.toXML();
    const generatedCreatedDate = select('/p:Document/p:CstmrCdtTrfInitn/p:GrpHdr/p:CreDtTm/text()', xml, true);
    expect(generatedCreatedDate.toString()).toBe('2023-07-29T01:02:03');

    const generatedExecutionDate = select('/p:Document/p:CstmrCdtTrfInitn/p:PmtInf/p:ReqdExctnDt/p:Dt/text()', xml, true);
  ...
  test('requested execution date stays the same in old pain versions for transfer documents', () => {
    ...
    const xml = doc.toXML();
    const generatedExecutionDate = select('/p:Document/p:CstmrCdtTrfInitn/p:PmtInf/p:ReqdExctnDt/text()', xml, true);
    expect(generatedExecutionDate.toString()).toBe('2025-07-29');
  });
});
