...
import { DOMImplementationClass, XMLSerializerClass } from './dom-implementation.js';
  ...
  var ID_SEPARATOR = '.';
  export function setIDSeparator(seperator) {
   ...
   */
  export function enableValidations(enabled, charsetValidationsEnabled = true) {
   ...
   * @constructor
   */
  export function Document(painFormat) {
  ...
  Document.prototype = {
    ...
    toString: function() {
      ...
      var docDeclaration = '<?xml version="' + this._xmlVersion + '" encoding="' + this._xmlEncoding + '"?>';
  ...
  SepaGroupHeader.prototype = {
    ...
    toXML: function(doc) {
      ...
      r(grpHdr, 'CreDtTm', toLocalDateTimeString(this.created));
  ...
  SepaPaymentInfo.prototype = {
    ...
    toXML: function(doc) {
      ...
      if (this.method === PaymentInfoTypes.DirectDebit) {
        ...
        r(pmtInf, 'ReqdColltnDt', toLocalCalendarDateString(this.collectionDate));
      }
      else {
        if (getPainXMLVersion(this._painFormat) >= 8) {
          var reqdExctnDt = n(pmtInf, 'ReqdExctnDt');
          r(reqdExctnDt, 'Dt', toLocalCalendarDateString(this.requestedExecutionDate));
        } else {
          r(pmtInf, 'ReqdExctnDt', toLocalCalendarDateString(this.requestedExecutionDate));
        }
      }
  ...
  SepaTransaction.prototype = {
    ...
    toXML: function(doc) {
      ...
      if (this._type === TransactionTypes.DirectDebit) {
        ...
        r(mandate, 'DtOfSgntr', toLocalCalendarDateString(this.mandateSignatureDate));
        ...
        var effectiveOriginalCreditorId = (this.amendment && this.amendment.originalCreditorSchemeId)
        ...
        if (effectiveOriginalCreditorId) {
          ...
          var amendmentDetails = n(mandate, 'AmdmntInfDtls');

          // Add original creditor scheme ID (for ICS migration)
          var originalCreditorScheme = n(amendmentDetails, 'OrgnlCdtrSchmeId', 'Id', 'PrvtId', 'Othr');
          r(originalCreditorScheme, 'Id', effectiveOriginalCreditorId);
   ...
   */
  export function checksumIBAN(iban) {
   ...
   */
  export function validateCreditorID(cid) {
   ...
   * @return            The corrected IBAN.
   */
  export function checksumCreditorID(cid) {
  ...
  function createDocument(nsURI, qname) {
    return new DOMImplementationClass().createDocument(nsURI, qname);
  }
...
/**
  ...
  function serializeToString(doc) {
    return new XMLSerializerClass().serializeToString(doc);
  }
